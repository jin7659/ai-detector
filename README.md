...
- **[안정성] 네트워크 독립형 모델 탑재**: Hugging Face 서버 접속 문제를 해결하기 위해, 도커 빌드 시점에 모델을 미리 내려받아 이미지에 내장(Pre-baked)했습니다. 이제 서버 기동 시 네트워크 에러가 발생하지 않습니다.
- **[안정성] 다중 클라이언트 접속 최적화**: 여러 기기나 브라우저에서 동시에 접속할 때 발생하던 `Already connected` 에러를 해결하기 위해 SSE 연결 관리 로직을 개선했습니다.
- **[안정성] 모델 교체**: 가장 신뢰도 높은 `Xenova/roberta-base-openai-detector`를 내장하여 탐지 성능과 안정성을 극대화했습니다.
